...
def load_matlab_file(path_file, name_field):
    ...
    try:
        if 'ir' in ds.keys():
            ...
            ir = np.asarray(ds['ir'])
            jc = np.asarray(ds['jc'])
            out = sp.csc_matrix((data, ir, jc)).astype(float)
    except AttributeError:
        # Transpose in case is a dense matrix because of the row- vs column- major ordering between python and matlab
        out = np.asarray(ds).astype(float).T
...
def create_trainvaltest_split(dataset, seed=1234, testing=False, datasplit_path=None, 
    ...
    # create labels
    all_labels = np.array([rating_dict[r] for r in ratings], dtype=int)
    ...
    rating_mx_train = sp.csr_matrix((data, [u_train_idx, v_train_idx]), 
                                    shape=[num_users, num_items], dtype=float)
...
def load_data_monti(dataset, testing=False, rating_map=None, post_rating_map=None):
    ...
    u_nodes_ratings, v_nodes_ratings = u_nodes_ratings.astype(int), v_nodes_ratings.astype(int)
    ...
    labels = np.full((num_users, num_items), neutral_rating, dtype=int)
    ...
    # make training adjacency matrix
    rating_mx_train = np.zeros(num_users * num_items, dtype=float)
    '''Note here rating matrix elements' values + 1 !!!'''
    if post_rating_map is None:
        rating_mx_train[train_idx] = labels[train_idx].astype(float) + 1.
    else:
...
def load_official_trainvaltest_split(dataset, testing=False, rating_map=None, post_rating_map=None, ratio=1.0):
    ...
    dtypes = {
        'u_nodes': int, 'v_nodes': int,
        'ratings': float, 'timestamp': float}
    ...
    if dataset =='ml_100k':
        ...
        v_features = np.zeros((num_items, num_genres), dtype=float)
        ...
        u_features = np.zeros((num_users, num_feats), dtype=float)
